...
$query = "SELECT sensor_logs.*, COALESCE(rooms.name, sensor_logs.room_id::text) AS room_name\n              FROM sensor_logs\n              LEFT JOIN rooms ON sensor_logs.room_id = rooms.id\n              ORDER BY measured_at DESC LIMIT 10";
    ...
    fetch('sensor.php?ajax=1')
        ...
        .then(rows=>{
            ...
            for(let r of rows){
                ...
                html += '<td>'+esc(r.room_name || r.room_id)+'</td>';
...
if (isset($_GET['ajax'])) {
    ...
    // Return rows as JSON for JS to render
    echo json_encode($rows);
    exit;
}

echo "<table>";
echo "<tr><th>Time</th><th>Room</th><th>Temp (&deg;C)</th><th>Humidity (%)</th><th>CO2 (ppm)</th><th>Illuminance (lx)</th></tr>";
foreach($rows as $r){
    echo "<tr>";
